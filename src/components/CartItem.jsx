...
const CartItem = ({ sd }) => {
  ...
  const renderCart = ({ item, index }) => {
    ...
    const itemTotalPrice = totalP[item.id] || totalCart.price;

    const { img, product } = item;
    return (
      <View key={item.toString()} style={{ padding: 10 }}>
        <View style={styles.itemContainer}>
          ...
          <View style={styles.itemTxtContainer}>
            ...
            <Text style={styles.sizeItem}>{totalCart.size}</Text>
...
// const CartItem = ({ sd }) => {
//   const { cart, setCart } = useContext(CartContext);
//   const [totalP, setTotalP] = useState({}); // Changed to an object to store each item's total price
...
//     // Update the totalP state for the current item
//     const itemTotalPrice = totalP[item.id] || totalCart.price;
